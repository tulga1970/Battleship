...
battleship.avtr = (function () {
  ...
  onTapNav = function ( event ){
    var css_map,
      $target = $( event.elem_target ).closest('.battleship-avtr-box');
  ...
  onHeldendNav = function ( event ) {
    ...
    $drag_target
      .removeClass('battleship-x-is-drag')
  ...
  onSetchatee = function ( event, arg_map ) {
    ...
    if ( old_chatee ){
      $nav
        .find( '.battleship-avtr-box[data-id=' + old_chatee.cid + ']' )
        .removeClass( 'battleship-x-is-chatee' );
    }

    // add highlight to new_chatee avatar here
    if ( new_chatee ){
      $nav
        .find( '.battleship-avtr-box[data-id=' + new_chatee.cid + ']' )
        .addClass('battleship-x-is-chatee');
    }
  };
  ...
  onListchange = function ( event ){
    ...
    people_db().each( function ( person, idx ){
      ...
      class_list = [ 'battleship-avtr-box' ];

      if ( person.id === chatee.id ){
        class_list.push( 'battleship-x-is-chatee' );
      }
      if ( person.get_is_user() ){
        class_list.push( 'battleship-x-is-user');
      }
      ...
      $box = $('<div/>')
        ...
        .prop( 'title', battleship.util_b.encodeHtml( person.name ))
        .text( person.name )
        .appendTo( $nav );
    });
  };
  ...
  // Example  : battleship.avtr.configModule({...});
  ...
  // Example    : battleship.avtr.initModule( $container );
  ...
  initModule = function ( $container ) {
    ...
    $.gevent.subscribe( $container, 'battleship-setchatee',  onSetchatee  );
    $.gevent.subscribe( $container, 'battleship-listchange', onListchange );
